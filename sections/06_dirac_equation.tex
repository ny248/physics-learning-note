...
\]
時間について1階で、かつ相対論的エネルギー・運動量関係と両立する方程式を作ると、係数は行列になり、波動関数はスピノールになる。
...
電子には、軌道角運動量とは別に二値の内部自由度があることが実験的に知られていた。たとえばシュテルン--ゲルラッハ実験では、中性原子のビームを不均一磁場に通すと、古典論なら連続的に広がるはずの像が二本に分かれる。これは磁気モーメントのある軸方向成分が離散的な二値しかとらないことを意味し、非相対論的理論ではこの自由度をパウリ行列で表す。ディラック方程式の重要な点は、ローレンツ共変な一次方程式を要請すると、係数行列の表現空間として複数成分の波動関数が必要になり、その正エネルギー部分に二つの独立な内部状態が現れることである。
上述の関係式を満たす行列を $2\times2$ 行列で作ろうとすると、互いに反交換して二乗が $1$ になる独立な行列を $\alpha_1,\alpha_2,\alpha_3,\beta$ の4個そろえることができない。ディラック代数の複素行列表現として必要な最低次元は4である。したがって状態は4成分複素ベクトル値の波動関数
\begin{equation}
...
自由粒子解を正エネルギー枝と負エネルギー枝に分けると、それぞれに2つの独立なスピン状態が現れる。場として量子化した後には、これが粒子と反粒子の各2自由度に対応し、電子のスピン $1/2$ を記述する。

\subsection{ディラック方程式の意義}
以上を踏まえると、ディラック方程式の意義は次の二点にまとめられる。
\begin{enumerate}
    \item \textbf{スピンの導入}：ローレンツ共変な一次方程式を要請すると、波動関数は4成分スピノールになり、その正エネルギー解には二つの独立な内部状態が現れる。この二値自由度が電子のスピン $1/2$ と対応する。
    \item \textbf{反粒子自由度の出現}：負エネルギー枝をもつことは一粒子解釈の限界を示す。場として量子化すると、負エネルギー枝に対応する自由度は陽電子を生成する演算子として現れる。
\end{enumerate}
