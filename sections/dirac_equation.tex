...
シュテルン--ゲルラッハ実験で見たように、電子には軌道角運動量とは別に二値の内部自由度がある。非相対論的理論ではこの自由度をパウリ行列で表す。ディラック方程式の重要な点は、ローレンツ共変な一次方程式を要請すると、係数行列の表現空間として複数成分の波動関数が必要になり、その正エネルギー部分に二つの独立な内部状態が現れることである。
