...
\]
固定粒子数の量子力学では、粒子数 $n$ ごとに別々のヒルベルト空間を用意し、同種粒子では交換に対する対称性を追加条件として課す。

ここまで見てきた相対論的一粒子方程式の困難は、「粒子数を固定したまま量子論を作る」という前提そのものを見直す必要があることを示している。しかし場の量子論へ一気に進む前に、固定粒子数の理論が通常の量子力学ではどのように組み立てられ、どこまで有効なのかを明確にしておきたい。そこでこの節では、テンソル積、対称化・反対称化、同種粒子の統計という、後にフォック空間へまとめ直される構造を整理する。
...
固定粒子数の理論では、このように粒子数 $n$ ごとに別々のヒルベルト空間を用意することになる。次節では、これらをすべてまとめた空間としてフォック空間を導入し、粒子の生成・消滅を最初から理論の内部に組み込む。
